...
export function useCollapsibleStyle() {
    const { headerHeight, tabBarHeight, containerHeight, width, allowHeaderOverscroll, minHeaderHeight, } = useTabsContext();
...
export function useUpdateScrollViewContentSize({ name }) {
...
export const useScrollHandlerY = (name) => {
    const { accDiffClamp, focusedTab, snapThreshold, revealHeaderOnScroll, refMap, tabNames, index, headerHeight, contentInset, containerHeight, scrollYCurrent, scrollY, oldAccScrollY, accScrollY, offset, headerScrollDistance, snappingTo, contentHeights, indexDecimal, allowHeaderOverscroll, } = useTabsContext();
        ...
        if (typeof snapThreshold === 'number') {
            if (revealHeaderOnScroll) {
                if (accDiffClamp.value > 0) {
                    if (scrollYCurrent.value >
                        headerScrollDistance.value * snapThreshold) {
                        if (accDiffClamp.value <=
                            headerScrollDistance.value * snapThreshold) {
                            // snap down
    ...
    const scrollHandler = useAnimatedScrollHandler({
        onScroll: (event) => {
            ...
            if (focusedTab.value === name) {
                if (IS_IOS) {
                    ...
                    const clampMax = contentHeight.value -
        ...
        onEndDrag: () => {
            ...
            if (IS_IOS) {
                ...
                afterDrag.value = withDelay(ONE_FRAME_MS, withTiming(0, { duration: 0 }, (isFinished) => {
    ...
    useAnimatedReaction(() => {
        ...
        if (isSyncNeeded &&
            ...
            if (!areEqual) {
                ...
                const focusedIsOnTop = focusedScrollY + StyleSheet.hairlineWidth <=
