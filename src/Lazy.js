 ...
 */
export const Lazy = ({ children, cancelLazyFadeIn, startMounted: _startMounted, mountDelayMs = 50, }) => {
    const name = useTabNameContext();
    const { focusedTab, refMap } = useTabsContext();
    /**
     * We start mounted if we are the focused tab, or if props.startMounted is true.
     */
    const startMounted = useSharedValue(typeof _startMounted === 'boolean'
        ? _startMounted
    ...
    return canMount ? (cancelLazyFadeIn ? (children) : (React.createElement(Animated.View, { pointerEvents: "box-none", style: [styles.container, !cancelLazyFadeIn ? stylez : undefined], onLayout: onLayout }, children))) : (React.createElement(ScrollView, null));
};
