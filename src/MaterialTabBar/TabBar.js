 ...
 * ```
 */
const MaterialTabBar = ({ tabNames, indexDecimal, scrollEnabled = false, indicatorStyle, index, TabItemComponent = MaterialTabItem, getLabelText = (name) => String(name).toUpperCase(), onTabPress, style, tabProps, contentContainerStyle, labelStyle, inactiveColor, activeColor, tabStyle, width: customWidth, keepActiveTabCentered, }) => {
    ...
    const [itemsLayout, setItemsLayout] = React.useState(scrollEnabled
        ? []
        : tabNames.map((_, i) => {
    ...
    return (React.createElement(Animated.ScrollView, { ref: tabBarRef, horizontal: true, style: style, contentContainerStyle: [
            styles.contentContainer,
            !scrollEnabled && { width },
            contentContainerStyle,
        ], keyboardShouldPersistTaps: "handled", bounces: false, alwaysBounceHorizontal: false, scrollsToTop: false, showsHorizontalScrollIndicator: false, automaticallyAdjustContentInsets: false, overScrollMode: "never", scrollEnabled: scrollEnabled, onScroll: scrollEnabled ? onScroll : undefined, scrollEventThrottle: 16 },
        tabNames.map((name, i) => {
            return (React.createElement(TabItemComponent, { key: name, index: i, name: name, label: tabProps.get(name)?.label || getLabelText(name), onPress: onTabPress, onLayout: scrollEnabled
                    ? (event) => onTabItemLayout(event, name)
                    : undefined, scrollEnabled: scrollEnabled, indexDecimal: indexDecimal, labelStyle: labelStyle, activeColor: activeColor, inactiveColor: inactiveColor, style: tabStyle }));
        }),
        itemsLayout.length === nTabs && (React.createElement(Indicator, { indexDecimal: indexDecimal, itemsLayout: itemsLayout, fadeIn: scrollEnabled, style: indicatorStyle }))));
};
