...
const Indicator = ({ indexDecimal, itemsLayout, style, fadeIn = false, }) => {
    ...
    const stylez = useAnimatedStyle(() => {
        const transform = itemsLayout.length > 1
            ? [
                {
                    translateX: interpolate(indexDecimal.value, itemsLayout.map((_, i) => i), 
        ...
        const width = itemsLayout.length > 1
            ? interpolate(indexDecimal.value, itemsLayout.map((_, i) => i), itemsLayout.map((v) => v.width))
            : itemsLayout[0]?.width;
        return {
            transform,
            width,
    ...
    return React.createElement(Animated.View, { style: [stylez, styles.indicator, style] });
};
