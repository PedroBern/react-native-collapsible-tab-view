...
export const Container = React.memo(React.forwardRef(({ initialTabName, headerHeight: initialHeaderHeight, minHeaderHeight = 0, tabBarHeight: initialTabBarHeight = TABBAR_HEIGHT, revealHeaderOnScroll = false, snapThreshold, children, renderHeader, renderTabBar = (props) => React.createElement(MaterialTabBar, Object.assign({}, props)), headerContainerStyle, cancelTranslation, containerStyle, lazy, cancelLazyFadeIn, pagerProps, onIndexChange, onTabChange, width: customWidth, allowHeaderOverscroll, }, ref) => {
    ...
    const index = useSharedValue(initialTabName
    ...
    return (React.createElement(Context.Provider, { value: {
        ...
        React.createElement(Animated.View, { style: [styles.container, { width }, containerStyle], onLayout: onLayout, pointerEvents: "box-none" },
            React.createElement(Animated.View, { pointerEvents: "box-none", style: [
                    styles.topContainer,
                    headerContainerStyle,
                    !cancelTranslation && stylez,
                ] },
                React.createElement(View, { style: [styles.container, styles.headerContainer], onLayout: getHeaderHeight, pointerEvents: "box-none" }, renderHeader &&
                    renderHeader({
                        containerRef,
                        index,
                ...
                React.createElement(View, { style: [styles.container, styles.tabBarContainer], onLayout: getTabBarHeight, pointerEvents: "box-none" }, renderTabBar &&
                    renderTabBar({
                        containerRef,
                        index,
                        tabNames: tabNamesArray,
            ...
            React.createElement(AnimatedPagerView, Object.assign({ ref: containerRef, onPageScroll: pageScrollHandler, initialPage: index.value }, pagerProps, { style: [pagerProps?.style, StyleSheet.absoluteFill] }), data.map((tabName, i) => {
                return (React.createElement(View, { key: i },
                    React.createElement(TabNameContext.Provider, { value: tabName },
                        React.createElement(Lazy, { startMounted: lazy ? undefined : true, cancelLazyFadeIn: !lazy ? true : !!cancelLazyFadeIn }, React.Children.toArray(children)[i]))));
            })))));
}));
