...
function generateProp(propName, prop, skipDefaults, skipDescription) {
...
function generateProps(props, isHook) {
    const skipDefaults = Object.keys(props)
        .map((p) => props[p].defaultValue)
        .filter((v) => v !== null).length === 0;
    const skipDescription = Object.keys(props)
    ...
    md += Object.keys(props)
        ...
        return generateProp(propName, props[propName], skipDefaults, skipDescription);
    })
        .join('\n');
    return md;
}
function generateMarkdown(api) {
