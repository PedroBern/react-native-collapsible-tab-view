...
| `onHeaderHeightChange?`     | Callback fired when the `headerHeight` state value inside `CollapsibleTabView` will be updated in the `onLayout` event from the tab/header container.<br/><br/> Useful to call layout animations. Example:<br/><br/><pre lang="js">() => {<br>LayoutAnimation.configureNext(LayoutAnimation.Presets.easeInEaseOut)};</pre> | `undefined`             |
